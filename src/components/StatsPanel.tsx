        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className={`w-full max-w-2xl ${theme.card} border ${theme.border} rounded-2xl p-6 shadow-2xl animate-popIn max-h-[85vh] overflow-y-auto`}>
            <div className="flex items-center justify-between mb-5">
              <h3 className={`text-base font-bold ${theme.accent}`}>{t('panel.title')}</h3>
            ...
            <div className={`grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 ${theme.sub}`}>
              {[
                { label: t('panel.totalKeys'), value: analytics.totalKeys },
                { label: t('panel.accuracyAll'), value: `${accuracyOf(analytics)}%` },
                { label: t('panel.totalWords'), value: analytics.totalWords },
                { label: t('panel.bestWpm'), value: analytics.bestWpm },
              ].map((it) => (
            ...
            {/* 易错字母 */}
            <div className="mb-6">
              <div className="text-sm font-semibold mb-3">{t('panel.weakKeys')}</div>
              {weak.length === 0 ? (
                <div className={`text-xs ${theme.sub}`}>{t('panel.noData')}</div>
              ) : (
                  ...
                  {weak.map((w) => (
                    <div key={w.letter} className="flex items-center gap-3">
                      ...
                      <span className={`text-[11px] tabular-nums ${theme.sub} w-16 text-right`}>
                        {t('panel.miss')} {w.miss} {t('panel.times')} · {Math.round(w.rate * 100)}%
                      </span>
                    </div>
                  ))}
                </div>
            ...
            {/* 错词榜 */}
            <div className="mb-6">
              <div className="text-sm font-semibold mb-3">{t('panel.wrongWords')}</div>
              {wrong.length === 0 ? (
                <div className={`text-xs ${theme.sub}`}>{t('panel.noWrong')}</div>
              ) : (
            ...
            <div className={`pt-4 border-t ${theme.border} flex items-center justify-between`}>
              <span className={`text-[11px] ${theme.sub}`}>{t('panel.localOnly')}</span>
              {confirmReset ? (
